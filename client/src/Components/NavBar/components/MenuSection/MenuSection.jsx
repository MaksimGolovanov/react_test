...
const MenuSection = ({ userRolesAuth }) => {
  ...
  // Используем useCallback для стабильной функции hasAccess
  const hasAccess = useCallback(
    (role) => userRolesAuth.includes(role) || userRolesAuth.includes('ADMIN'),
    [userRolesAuth]
  );

  // Функция для определения openKeys (обёрнута в useMemo, зависит от hasAccess)
  ...
  const menuItems = useMemo(() => {
    ...
    if (hasAccess('CONSUMABLES')) {
      items.push({
        key: '/consumables',
        icon: <InboxOutlined />,
        label: <Link to="/consumables">РАСХОДНЫЕ МАТЕРИАЛЫ</Link>,
      });
    }
    if (hasAccess('IP')) {
      items.push({
    ...
    /*
    if (hasAccess('PROTOCOLS')) {
      items.push({
        key: '/protocols',
        icon: <FileTextOutlined  />,
        label: <Link to="/protocols">ПРОТОКОЛ ОТ</Link>,
      });
    }*/
    if (hasAccess('GRAMOTA')) {
      items.push({
        key: '/gramota',
        icon: <FileTextOutlined  />,
        label: <Link to="/gramota">ГРАМОТЫ</Link>,
      });
    }
    if (hasAccess('PHOTOEDITOR')) {
      items.push({
        key: '/photoeditor',
        icon: <PictureOutlined />,
        label: <Link to="/photoeditor">ФОТО</Link>,
      });
    }
   /* if (hasAccess('FLOORPLAN')) {
      items.push({
        key: '/floorplan',
        icon: <PictureOutlined />,
        label: <Link to="/floorplan">ПЛАН</Link>,
      });
    }
      */
  /*   if (hasAccess('PLANS')) {
      items.push({
        key: '/plans',
        icon: <UsbOutlined />,
        label: <Link to="/plans">ПЛАН ОР</Link>,
      });
    }
   if (hasAccess('NARYAD')) {
      items.push({
        key: '/naryad',
        icon: <UsbOutlined />,
        label: <Link to="/naryad">НАРЯД ДОПУСК</Link>,
      });
    }*/
    if (hasAccess('CARD')) {
      items.push({
