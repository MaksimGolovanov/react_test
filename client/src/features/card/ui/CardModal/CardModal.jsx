...
import { Modal, Form, Input, Select, Button, Space, AutoComplete, theme } from "antd";
...
const CardModal = ({
  ...
  return (
    <Modal
      ...
      width={800}
    >
      <Form form={form} onFinish={onFinish} layout="vertical" initialValues={formData}>
        <div style={{ display: "flex", gap: "16px" }}>
          <div style={{ flex: 1 }}>
            ...
            <Form.Item label="Описание" name="description">
              <Select placeholder="Выберите описание" allowClear>
          ...
          <div style={{ flex: 1 }}>
            <Form.Item
              label={
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span>ФИО</span>
                  <Button type="link" size="small" onClick={onClearFio} disabled={!formData?.fio && !formData?.department}>
                    Очистить
                  </Button>
                </div>
              }
            ...
            <Form.Item label="Служба" name="department">
              <Input readOnly placeholder="Заполнится автоматически" />
            </Form.Item>
            <Form.Item label="Дата проверки" name="data_prov">
              <Input type="date" />
            </Form.Item>
          </div>
        </div>
        <Form.Item label="В работе" name="log" initialValue="Да">
          <Select>
