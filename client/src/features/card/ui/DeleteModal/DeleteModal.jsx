...
import { Modal, Button, Alert, List, Typography, Space, theme } from "antd";
import { ExclamationCircleOutlined } from "@ant-design/icons";

const { Text } = Typography;
const { useToken } = theme;

const DeleteModal = ({ show, onHide, selectedIds, selectedCardsInfo, onConfirm }) => {
      ...
      {selectedIds.length === 1 ? (
        ...
        <>
          ...
          <List
            ...
            renderItem={(card) => <List.Item>• {card.ser_num || "Без номера"} - {card.fio || "Без ФИО"}</List.Item>}
          />
          {selectedIds.length > 5 && <p style={{ marginTop: "8px" }}>... и еще {selectedIds.length - 5} записей</p>}
        </>
      )}
      <Alert
        message="Внимание!"
        ...
        type="warning"
        showIcon
        style={{ marginTop: "16px", background: token.colorWarningBg, borderColor: token.colorWarningBorder }}
      />
    </Modal>
  );
};
