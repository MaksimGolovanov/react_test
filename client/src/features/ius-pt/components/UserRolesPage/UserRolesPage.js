  ...
  const handleFileUpload = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Получаем все коды из первого столбца файла Excel
        const codesFromExcel = json.map((row) => {
          // Берем значение из первого столбца (может быть под разными именами)
          const firstColumnKey = Object.keys(row)[0];
          return String(row[firstColumnKey]).trim();
        });

        // Находим роли, чьи коды есть в файле Excel
        ...
        alert(`Успешно загружено ${matchedRoles.length} ролей из файла!`);
      } catch (error) {
        console.error("Ошибка при обработке файла:", error);
        alert(
          "Ошибка при обработке файла. Убедитесь, что файл имеет правильный формат и содержит коды ролей в первом столбце."
        );
      }
    };
    ...
    <div className={styles.container}>
      ...
      <div className={styles.fileUpload}>
        ...
        <div className={styles.uploadHint}>
          Файл должен содержать коды ролей в первом столбце
        </div>
      </div>
    </div>
  );
