...
const IusUserApplication = observer(() => {
  ...
  const iusadm = iusPtStore.admins.find(
    (admin) => admin.cod === "admarm"
  ).iusadm;
  const iusadmemail = iusPtStore.admins.find(
    (admin) => admin.cod === "admarm"
  ).email;
  const iusib = iusPtStore.admins.find((admin) => admin.cod === "admib").iusadm;
  const iusibemail = iusPtStore.admins.find(
    (admin) => admin.cod === "admib"
  ).email;
  ...
  const groupedRoles = () => {
    ...
    roles.forEach((role) => {
      ...
      // Используем ISO-дату для единообразия
      ...
      if (!groups[groupKey]) {
        groups[groupKey] = {
          ...
          type,
          roles: [],
          createdAt: createdAt.getTime(), // Сохраняем timestamp для сортировки
          date: displayDate,
        };
      }
  ...
  console.log("Original roles:", user?.IusUser?.IusSpravRoles);
  ...
  const exportToExcel = async () => {
    ...
    selectedGroup.roles
      ...
      .forEach((role, index) => {
        ...
        worksheet.getCell(`A${index + 19}`).alignment = {
          horizontal: "center",
          vertical: "middle",
        };
        worksheet.getCell(`D${index + 19}`).value = "Добавить";
        worksheet.getCell(`D${index + 19}`).alignment = {
          horizontal: "center",
          vertical: "middle",
        };
        worksheet.getCell(`H${index + 19}`).value =
          role.type + "/" + role.mandat;
        worksheet.getCell(`H${index + 19}`).alignment = {
          horizontal: "center",
          vertical: "middle",
        };
        worksheet.getCell(`N${index + 19}`).value = role.name;
        worksheet.getCell(`N${index + 19}`).alignment = {
          horizontal: "left",
          vertical: "middle",
          wrapText: true,
        };
        worksheet.getCell(`Z${index + 19}`).value = role.code;
        worksheet.getCell(`Z${index + 19}`).alignment = {
          horizontal: "left",
          vertical: "middle",
          wrapText: true,
        };
    ...
    const footer = [
      ...
      },
      {
        ...
        text: `Изменение условий подключения АРМ ${
          checkboxes.conditionsChange ? "☑" : "☐"
        }`,
        size: "9",
        horizontal: "left",
        hasBorder: true,
          ...
          {groupedRoles().map((group) => (
            <div
              ...
              className={`${styles.groupItem} ${
                selectedGroup?.key === group.key ? styles.selected : ""
              }`}
            ...
            >
              ...
              <div className={styles.groupDetails}>
                ...
                <span className={styles.rolesCount}>
                  {group.roles.length} {getRoleWord(group.roles.length)}
                </span>
              </div>
            </div>
          ))}
