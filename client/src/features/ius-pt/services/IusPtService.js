...
const instance = axios.create({
  baseURL: `${API_URL}api`,
  timeout: 15000, // Таймаут 5 секунд
});
...
class IusPtService {
  ...
  static addRolesToUser = async (tabNumber, roleIds) => {
    return this.request('post', '/iuspt/user-roles/bulk', {
      tabNumber,
      roleIds,
    });
  };
  ...
  static fetchStaffByTabNumber = async (tabNumber) => {
    return this.request(
      'get',
      `/iuspt/staff-with-iususer-tabnumber/${tabNumber}`
    );
  };
  static fetchStaffWithIusUserSimple = async (search = '') => {
    const query = search ? `?search=${encodeURIComponent(search)}` : '';
    return this.request('get', `/iuspt/staff-with-iususer-simple${query}`);
  };
  ...
  static bulkCreateStopRoles = async (stopRoles) => {
    return this.request('post', '/iuspt/stoproles/bulk', stopRoles);
  };

  static bulkDeleteStopRoles = async (ids) => {
    return this.request('delete', '/iuspt/stoproles/bulk', { ids });
  };

  static fetchTransactions = async (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return this.request('get', `/iuspt/transactions?${query}`);
  };

  static bulkCreateTransactions = async (transactions) => {
    return this.request('post', '/iuspt/transactions/bulk', transactions);
  };

  static bulkDeleteTransactions = async (ids) => {
    return this.request('delete', '/iuspt/transactions/bulk', { ids });
  };

  static addRoleToTransaction = async (transactionId, roleId) => {
    return this.request('post', '/iuspt/transaction-role', {
      transactionId,
      roleId,
    });
  };

  static removeRoleFromTransaction = async (transactionId, roleId) => {
    return this.request('delete', '/iuspt/transaction-role', {
      transactionId,
      roleId,
    });
  };
  static createTransaction = async (data) => {
    return this.request('post', '/iuspt/transactions', data);
  };

  static updateTransaction = async (id, data) => {
    return this.request('put', `/iuspt/transactions/${id}`, data);
  };

  static deleteTransaction = async (id) => {
    return this.request('delete', `/iuspt/transactions/${id}`);
  };

  static getStaffByRole = async (roleId) => {
    return this.request('get', `/iuspt/staff-by-role/${roleId}`);
  };
}
