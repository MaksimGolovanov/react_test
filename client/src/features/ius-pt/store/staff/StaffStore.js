...
export class StaffStore extends BaseStore {
    ...
    fetchStaffWithIusUsers = async () => {
        try {
            const response = await super.fetchData(IusPtService.fetchStaffWithIusUser, 'staffWithIusUsers')
            runInAction(() => {
    ...
    fetchStaffWithIusUserSimple = async () => {
        try {
            const response = await super.fetchData(IusPtService.fetchStaffWithIusUserSimple, 'staffWithIusUsersSimple')
            runInAction(() => {
                this.staffWithIusUsersSimple = response
            })
        } catch (error) {
            console.error('Ошибка при получении упрощенных данных сотрудников:', error)
            throw error
        }
    }
    ...
    fetchStaffWithIusUserSimpleOver = async () => {
        try {
            const response = await super.fetchData(IusPtService.fetchStaffWithIusUserSimpleOver, 'staffWithIusUsersSimpleOver')
            runInAction(() => {
