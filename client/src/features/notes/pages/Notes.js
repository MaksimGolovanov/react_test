...
function Notes() {
    ...
    const editPost = id => {
        ...
        navigate(`/edit-post/${id}`); // Переход на страницу редактирования
    };
        ...
        <Container>
            ...
            <Row>
                <NavLink to="/create-post" >
                    <Button
                        className='button-next float-end ms-auto mb-3'
                            ...
                            <Card.Body>
                                <Card.Title>
                                    ...
                                    <div style={{ float: 'right' }}>
                                        <FaEdit className="edit-icon mr-2 " size={14} title="Редактировать" onClick={() => editPost(post.id)} />
                                        <FaTrashAlt className="delete-icon ml-2" size={14} title="Удалить" onClick={() => deletePost(post.id)} />
                                    </div>
                                </Card.Title>
                                <Card.Text dangerouslySetInnerHTML={{
                                    __html: draftToHtml(post.body).replace(
                                        /(<pre>)/g, `<span class="code-block">$1<span class="copy-button"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></span></span>`
                                    )
                                }}>
                                </Card.Text>
                            </Card.Body>
