...
export default function PrintEditModal({
  isOpen,
  onRequestClose,
  onSuccess,
  printData,
  ...
  // Заполняем форму, когда загружены справочники и есть данные принтера
  useEffect(() => {
    if (initialized && printData) {
      form.setFieldsValue({
        model_id: Number(printData.print_model),
        ...
        status: printData.status ?? 1,
      });
    }
  }, [initialized, printData, form]);
            ...
            >
              <Select allowClear>
                {departments.map((dept) => (
                  <Select.Option key={dept.id} value={dept.short_name}>
                    {dept.short_name}
                  </Select.Option>
                ))}
              </Select>
        ...
        <Row gutter={16}>
          <Col span={12}>
            <Form.Item name="location" label="Расположение">
              <Select allowClear>
                {locations.map((loc) => (
                  <Select.Option key={loc.id} value={loc.location}>
                    {loc.location}
                  </Select.Option>
                ))}
              </Select>
