...
class SecurityTrainingStore {
     ...
     testResults = []
     isLoading = false
     users = observable.array([]) // Изменение здесь
     roles = []
     userRoles = []
     userRolesAuth = []
     loading = true
    ...
    fetchUsers = action(async () => {
          try {
               this.loading = true
               const responseUsers = await STService.fetchUser()
               const responseRoles = await STService.fetchRole()
               const responseUserRole = await STService.fetchRoleUser()

               console.log('Fetched users from API:', responseUsers.length)

               // Заменяем весь массив (это вызовет реакцию)
               this.users.replace(responseUsers)
               
               this.roles = responseRoles
               this.userRoles = responseUserRole
               
               console.log('Users in store after fetch:', this.users.length)
          } catch (error) {
     ...
     saveProgress = () => {
          try {
               localStorage.setItem('securityTrainingProgress', JSON.stringify(this.userProgress))
          } catch (error) {
