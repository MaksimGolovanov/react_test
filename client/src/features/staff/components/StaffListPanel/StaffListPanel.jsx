...
import { Input, Typography, theme, Tag } from 'antd';
...
function StaffListPanel({
  ...
  const renderUserStatus = (user) => {
    ...
    if (user.isDeleted) {
      return (
        <span
          className={styles.deletedLabel}
          style={{ backgroundColor: token.colorError, color: '#fff' }}
        >
          Уволен
        </span>
      );
  ...
  const getEmptyText = () => {
    if (filterDepartment)
      return `В отделе «${filterDepartment}» нет сотрудников`;
              ...
              <div className={styles.titleRow}>
                ...
                {filterDepartment && (
                  <Tag
                    color="blue"
                    closable
                    onClose={() => onDepartmentClick(null)}
                    style={{ marginLeft: 8 }}
                  >
                    {filterDepartment}
                  </Tag>
                ...
                >
                  ...
                  <div className={styles.itemInfo}>
                    ...
                    <div className={styles.departmentRow}>
                      <Text
                        className={styles.itemDepartment}
                        style={{
                          color: token.colorTextTertiary,
                          cursor: 'pointer',
                          textDecoration:
                            filterDepartment === user.departmentName
                              ? 'underline'
                              : 'none',
                        }}
                        onClick={(e) => {
                          e.stopPropagation(); // чтобы не сработал выбор сотрудника
                          if (filterDepartment === user.departmentName) {
                            onDepartmentClick(null); // сброс фильтра
                          } else {
                            onDepartmentClick(user.departmentName);
                          }
                        }}
                      >
