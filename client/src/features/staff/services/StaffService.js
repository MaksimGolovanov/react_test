...
class StaffService {
  ...
  static async updateStaff(updatedStaff) {
    try {
      await axios.put(
        `${API_URL}api/staff/${updatedStaff.tabNumber}`,
        updatedStaff
      );
    } catch (error) {
  ...
  static async uploadPhoto(tabNumber, file) {
    try {
      ...
      const response = await axios.post(
        `${API_URL}api/staff/upload-photo`,
        formData,
        {
          headers: {
  ...
  static async fetchUsbByFio(fio) {
    try {
      ...
      console.error('Ошибка при получении USB:', error);
      throw error;
    }
  }
}
