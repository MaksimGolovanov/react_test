...
class IusSpravAdmController {
    ...
    async create(req, res, next) {
        try {
            const { iusadm, description, email, cod } = req.body;
            if (!iusadm) {
                return next(ApiError.badRequest('Не указаны обязательные поля'));
            }
            const adm = await IusSpravAdm.create({ iusadm, description, email, cod  });
    ...
    async update(req, res, next) {
        try {
            const { id, iusadm, description, email, cod } = req.body;
            if (!id || !iusadm) {
                return next(ApiError.badRequest('Не указаны обязательные поля'));
            }
