...
class PrintController {
     async create(req, res, next) {
          try {
               ...
               const { print_model, logical_name, ip, url, department, location, serial_number, status, description } =
                    req.body
     ...
     async createStatistic(req, res) {
          const { itemid, clock, value } = req.body
          try {
               const stat = await PrinterStatistics.create({ itemid, clock, value })
     ...
     async updateStatistic(req, res) {
          const { id } = req.params
          const { clock, value } = req.body
          try {
               const stat = await PrinterStatistics.findByPk(id)
               if (!stat) return res.status(404).json({ message: 'Запись не найдена' })
               await stat.update({ clock, value })
               return res.json(stat)
          } catch (error) {
               return res.status(500).json({ error: error.message })
          }
