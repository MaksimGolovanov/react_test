...
class CourseController {
     ...
     async getAllCourses(req, res, next) {
          try {
               const { page = 1, limit = 10, level, category, active = true, search } = req.query
     ...
     static async submitTestResult(userId, courseId, testData) {
          try {
               ...
               const response = await axios.post(`${API_URL}api/courses/${courseId}/submit-test/${userId}`, testData, {
                    headers: {
     ...
     async createLesson(req, res, next) {
          try {
               ...
               const lesson = await Lesson.create({
                    ...
                    order_index,
                    is_active,
                    additional_resources: Array.isArray(additional_resources) ? additional_resources : [],
               })
          ...
          try {
               ...
               if (question_type === 'single' || question_type === 'multiple') {
                    if (!Array.isArray(options) || options.length < 2) {
                         return next(
                              ApiError.badRequest('Для этого типа вопроса требуется как минимум 2 варианта ответа')
                         )
                    }
                    ...
                    if (question_type === 'single' && correctOptions.length !== 1) {
                         return next(
                              ApiError.badRequest('Для одиночного выбора должен быть ровно один правильный ответ')
                         )
                    }

                    if (question_type === 'multiple' && correctOptions.length < 1) {
                         return next(
                              ApiError.badRequest('Для множественного выбора должен быть хотя бы один правильный ответ')
                         )
                    }
               }
          ...
          try {
               ...
               if (updateData.options) {
                    ...
                    if (questionType === 'single' || questionType === 'multiple') {
                         ...
                         const correctOptions = updateData.options.filter((opt) => opt.correct)
     ...
     // Получение статистики по курсу - УПРОЩЕННАЯ ВЕРСИЯ БЕЗ USER
     async getCourseStats(req, res, next) {
          try {
               ...
               // Последние результаты БЕЗ данных пользователя (временно)
               const recentResults = await TestResult.findAll({
                    where: { course_id: id },
                    order: [['created_at', 'DESC']],
                    limit: 10,
                    attributes: ['id', 'user_id', 'score', 'passed', 'time_spent', 'attempt_number', 'created_at'],
               })

               console.log('Recent results (simplified):', recentResults)
               ...
               return res.json({
                    ...
                    stats: {
                         ...
                         passing_rate: attemptsCount > 0 ? ((completedCount / attemptsCount) * 100).toFixed(2) : 0,
                    },
