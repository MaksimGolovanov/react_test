...
class UserProgressController {
     ...
     async updateSTUserStats(userId, courseId, progressData) {
          try {
               ...
               const totalScore = completedCourses.reduce(
                    (sum, course) => sum + (parseFloat(course.test_score) || 0),
                    0
               )
     ...
     async resetProgressByTabNumber(req, res, next) {
          try {
               const { tabNumber, courseId } = req.params

               console.log(`=== RESET PROGRESS BY TAB NUMBER ===`)
               console.log(`Tab Number: ${tabNumber}, Course ID: ${courseId}`)

               // 1. Преобразуем табельный номер в числовой формат (убираем ведущий ноль)
               let numericUserId

               // Если табельный номер начинается с нуля, убираем его
               if (tabNumber.startsWith('0')) {
                    numericUserId = parseInt(tabNumber, 10) // Это преобразует "02209873" в 2209873
               } else {
                    numericUserId = parseInt(tabNumber, 10)
               }

               console.log(`Numeric user ID: ${numericUserId}`)

               if (isNaN(numericUserId)) {
                    return next(ApiError.badRequest('Неверный формат табельного номера'))
               }

               // 2. Проверяем, существует ли STUser с таким табельным номером
               const stUser = await STUser.findOne({
                    where: { tabNumber: tabNumber },
               })

               if (!stUser) {
                    // Или пробуем найти без ведущего нуля
                    const altStUser = await STUser.findOne({
                         where: { tabNumber: numericUserId.toString() },
                    })

                    if (!altStUser) {
                         return next(
                              ApiError.notFound(`Пользователь с табельным номером ${tabNumber} не найден в STUser`)
                         )
                    }
               }

               // 3. Находим прогресс пользователя по курсу
               const progress = await UserCourseProgress.findOne({
                    where: {
                         user_id: numericUserId, // Используем числовой ID
                         course_id: courseId,
                    },
               })
               ...
               // 4. Удаляем ВСЕ результаты тестов пользователя по этому курсу
               const testResultsDeleted = await TestResult.destroy({
                    where: {
                         user_id: numericUserId, // Используем числовой ID
                         course_id: courseId,
                    },
               ...
               console.log(`Deleted ${testResultsDeleted} test results`)

               let progressUpdated = false
               let stUserUpdated = false

               // 5. Если прогресс существовал, полностью сбрасываем его
               if (progress) {
                    // Запоминаем старые данные
                    const wasCompleted = progress.passed_test
                    const oldTestScore = progress.test_score || 0
                    const oldTimeSpent = progress.total_time_spent || 0

                    // Сбрасываем ВСЕ данные прогресса
                    await progress.update({
                         completed_lessons: [],
                         test_score: 0,
                         passed_test: false,
                    ...
                    console.log('Progress reset to initial state:', {
                         before: {
                              ...
                              test_score: oldTestScore,
                              passed_test: wasCompleted,
                              total_time_spent: oldTimeSpent,
                         },
                    })

                    // 6. Обновляем статистику STUser если курс был завершен
                    if (wasCompleted) {
                         const stUserToUpdate =
                              stUser ||
                              (await STUser.findOne({
                                   where: {
                                        tabNumber: {
                                             [Op.or]: [tabNumber, numericUserId.toString()],
                                        },
                                   },
                              }))

                         if (stUserToUpdate) {
                              // Пересчитываем статистику STUser
                              const allProgresses = await UserCourseProgress.findAll({
                                   where: { user_id: numericUserId },
                              })

                              // Считаем только завершенные курсы
                              const completedCourses = allProgresses.filter((p) => p.passed_test)
                              const completedCount = completedCourses.length
                              ...
                              // Считаем общее время обучения
                              const totalTime = allProgresses.reduce((sum, p) => sum + (p.total_time_spent || 0), 0)

                              // Находим дату последнего завершенного курса
                              let lastCompleted = null
                              if (completedCount > 0) {
                                   const lastCourse = completedCourses.reduce((latest, current) => {
                                        const currentDate = new Date(current.completed_at || 0)
                                        const latestDate = new Date(latest.completed_at || 0)
                                        return currentDate > latestDate ? current : latest
                                   })
                                   lastCompleted = lastCourse.completed_at
                              }

                              // Обновляем STUser
                              await stUserToUpdate.update({
                                   completed_courses: completedCount,
                                   average_score: averageScore,
                                   total_training_time: totalTime,
                                   last_course_completed: lastCompleted,
                                   updated_at: new Date(),
                              })

                              stUserUpdated = true

                              console.log('STUser stats updated:', {
                                   completed_courses: completedCount,
                    ...
                    // Если прогресса не было, создаем пустую запись
                    ...
                    console.log('Created new empty progress record')
               }
               ...
               console.log(`STUser updated: ${stUserUpdated}`)

               return res.json({
                    success: true,
                    message: 'Статистика курса и тестов успешно сброшена',
                    user: {
                         tabNumber: tabNumber,
                    ...
                    details: {
                         ...
                         st_user_updated: stUserUpdated,
                         timestamp: new Date().toISOString(),
                    },
               })
          } catch (error) {
               console.error('Error in resetProgressByTabNumber:', error)
               return next(ApiError.internal('Ошибка при сбросе статистики курса: ' + error.message))
          }
     }
     ...
     async getUserStats(req, res, next) {
          try {
               ...
               const averageScore =
                    completedCourses > 0
                         ? progresses.filter((p) => p.passed_test).reduce((sum, p) => sum + (p.test_score || 0), 0) /
                           completedCourses
                         : 0
